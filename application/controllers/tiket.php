...
class Tiket extends CI_Controller
{
	...
	public function scan2()
	{
		...
		foreach ($harga1 as $value) {
			$harga = $value->harga_tiket;
		}
		foreach ($lokasi1 as $value) {
			$lokasi = $value->lokasi_parkir;
		}
		$id = rand();
	...
	public function pdf()
	{
		...
		$data['tiket'] =  $this->db->query("SELECT T.id_transaksi, A.nama_lengkap, T.lokasi_parkir FROM transaksi T INNER JOIN akun A ON T.id_user = A.nomor_identitas WHERE T.id_transaksi = '$where'")->result();
