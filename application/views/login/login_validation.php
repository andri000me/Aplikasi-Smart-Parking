  ...
  <div class="container" >          
    <form action="<?php echo base_url().'Login/login_process' ?>" method="post">
      ...
      <div class="input-group" style="margin-bottom: 10px;">
        <div class="input-group-text <?=form_error('no_telp') ? 'border2' : 'null'?>" id="basic-addon1" style="height: 45px; margin-top: 0px;">
          <span><i class="bi bi-telephone-fill"></i></span>
        </div>
        <input type="text" name="no_telp" id="no_telp" class="form-control <?=form_error('no_telp') ? 'is-invalid' : 'null'?>"  placeholder="Email atau Nomor Telepon" aria-label="Nomor Telepon" aria-describedby="basic-addon1">
      </div>
      <div class="pesan"><?= form_error('no_telp')?></div>

      <div class="input-group" style="margin-bottom: 10px;">
        <div class="input-group-text <?=form_error('password') ? 'border2' : 'null'?>" id="basic-addon1" style="height: 45px; margin-top: 0px;">
          <span><i class="bi bi-lock-fill"></i></span>
        </div>
        <input type="password" class="form-control <?=form_error('password') ? 'is-invalid' : 'null'?>" id="password" name="password" placeholder="Kata Sandi" aria-label="sandi" aria-describedby="basic-addon1">
      </div>
